...
export default function ProgressTracker() {
  ...
  const modules = ['MOD-01', 'MOD-02', 'MOD-03', 'MOD-04']

  return (
    <>
      {justEarned > 0 && (
        <div style={{ position: 'fixed', top: '80px', right: '24px', zIndex: 2000, background: 'rgba(0,255,65,0.12)', border: '1px solid rgba(0,255,65,0.4)', borderRadius: '6px', padding: '8px 16px', fontFamily: 'JetBrains Mono, monospace', fontSize: '13px', color: '#00ff41', animation: 'fadeUp 2s ease forwards' }}>
          +{justEarned} XP
          <style>{`@keyframes fadeUp{0%{opacity:1;transform:translateY(0)}100%{opacity:0;transform:translateY(-30px)}}`}</style>
        </div>
      )}

      <button onClick={() => setOpen(!open)} style={{ position: 'fixed', bottom: '90px', left: '24px', zIndex: 1000, background: 'rgba(0,255,65,0.08)', border: '1px solid rgba(0,255,65,0.3)', borderRadius: '6px', padding: '6px 12px', cursor: 'pointer', fontFamily: 'JetBrains Mono, monospace', fontSize: '9px', color: '#00ff41', letterSpacing: '0.1em', display: 'flex', alignItems: 'center', gap: '6px' }}>
        <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#00ff41' }} />
        {progress.xp} XP · {rank.title.toUpperCase()}
      </button>

      {open && (
        <div style={{ position: 'fixed', bottom: '130px', left: '24px', zIndex: 999, width: '340px', maxHeight: '520px', background: '#080c0a', border: '1px solid rgba(0,255,65,0.2)', borderRadius: '10px', display: 'flex', flexDirection: 'column', fontFamily: 'JetBrains Mono, monospace', overflow: 'hidden' }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid #1a2e1e', background: 'rgba(0,255,65,0.04)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', marginBottom: '6px' }}>
              <span style={{ fontSize: '22px', fontWeight: 700, color: rank.color }}>{progress.xp.toLocaleString()}</span>
              <span style={{ fontSize: '9px', color: '#3a6a3a', letterSpacing: '0.1em' }}>XP</span>
              <span style={{ fontSize: '10px', color: rank.color, marginLeft: 'auto', letterSpacing: '0.1em' }}>{rank.title.toUpperCase()}</span>
            </div>
            {nextRank && (
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                  <span style={{ fontSize: '8px', color: '#3a6a3a' }}>{xpToNext} XP to {nextRank.title}</span>
                  <span style={{ fontSize: '8px', color: '#3a6a3a' }}>{progressPct}%</span>
                </div>
            ...
            <div style={{ display: 'flex', gap: '4px', marginTop: '10px' }}>
              {(['progress', 'notes'] as const).map(t => (
                <button key={t} onClick={() => setTab(t)} style={{ background: tab === t ? 'rgba(0,255,65,0.1)' : 'transparent', border: `1px solid ${tab === t ? 'rgba(0,255,65,0.3)' : '#1a2e1e'}`, borderRadius: '3px', padding: '3px 10px', cursor: 'pointer', fontSize: '8px', color: tab === t ? '#00ff41' : '#3a6a3a', letterSpacing: '0.1em' }}>{t.toUpperCase()}</button>
              ))}
            </div>
          </div>
              ...
              return (
                <div key={mod} style={{ marginBottom: '12px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                    ...
                    <span style={{ fontSize: '8px', color: '#3a6a3a' }}>{modDone}/{modLabs.length}</span>
                  </div>
                  {modLabs.map(lab => {
                    const done = progress.completedLabs.includes(lab.id)
                    return (
                      <div key={lab.id} onClick={() => toggleLab(lab.id, lab.xp)} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '5px 6px', borderRadius: '3px', cursor: 'pointer', marginBottom: '2px', background: done ? 'rgba(0,255,65,0.04)' : 'transparent', border: `1px solid ${done ? 'rgba(0,255,65,0.15)' : 'transparent'}` }}>
                        <div style={{ width: '12px', height: '12px', borderRadius: '2px', border: `1px solid ${done ? color : '#1a2e1e'}`, background: done ? color + '22' : 'transparent', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          {done && <span style={{ fontSize: '8px', color }}>✓</span>}
                        </div>
                        <span style={{ fontSize: '0.68rem', color: done ? '#8a9a8a' : '#3a6a3a', flex: 1 }}>{lab.label}</span>
                        <span style={{ fontSize: '8px', color: done ? color : '#2a4a2a' }}>+{lab.xp}</span>
                      </div>
                    )
                  })}
                </div>
            ...
            {tab === 'notes' && (
              <div>
                <div style={{ display: 'flex', gap: '4px', marginBottom: '8px', flexWrap: 'wrap' as const }}>
                  {modules.map(m => (
                    <button key={m} onClick={() => { setNoteModule(m); setNoteText(progress.notes[m] || '') }} style={{ background: noteModule === m ? `${MOD_COLORS[m]}15` : 'transparent', border: `1px solid ${noteModule === m ? MOD_COLORS[m] + '44' : '#1a2e1e'}`, borderRadius: '3px', padding: '2px 8px', cursor: 'pointer', fontSize: '7px', color: noteModule === m ? MOD_COLORS[m] : '#3a6a3a', letterSpacing: '0.1em' }}>{m}</button>
                  ))}
                </div>
                <textarea
                  value={noteText}
                  ...
                  rows={10}
                  style={{ width: '100%', background: 'rgba(0,255,65,0.03)', border: '1px solid #1a2e1e', borderRadius: '4px', padding: '8px', color: '#8a9a8a', fontSize: '0.7rem', fontFamily: 'JetBrains Mono, monospace', resize: 'vertical' as const, outline: 'none', lineHeight: 1.6 }}
                />
                <button onClick={saveNote} style={{ marginTop: '6px', background: 'rgba(0,255,65,0.08)', border: '1px solid rgba(0,255,65,0.3)', borderRadius: '3px', padding: '5px 16px', cursor: 'pointer', fontSize: '8px', color: '#00ff41', letterSpacing: '0.1em', width: '100%' }}>SAVE NOTES</button>
              </div>
            )}
          </div>
        </div>
