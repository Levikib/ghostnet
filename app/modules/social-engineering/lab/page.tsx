...
What free online tool can detect whether a profile photo is AI-generated by analysing pixel-level artefacts and GAN (Generative Adversarial Network) fingerprints?`,
  ...
  // ── PHASE 6: PRETEXTING & PHYSICAL SE ────────────────────────────────────

  {
    id: 'se-13',
    title: 'Phase 6 — Pretexting Scripts: IT Help Desk Impersonation',
    objective: `Pretexting is building a fabricated scenario (a pretext) to manipulate a target into taking an action. The IT help desk impersonation is the most common and effective corporate SE attack — nearly every organisation has a help desk, and employees are conditioned to comply with IT requests. You call an employee pretending to be IT support, claim their account has been flagged for suspicious activity, and ask them to verify their identity by confirming their credentials. What psychological principle does the phrase "I need to verify your identity to protect your account" exploit?`,
    hint: 'The target is being asked to comply for their own protection — this exploits a specific Cialdini principle.',
    answers: ['authority', 'fear', 'urgency', 'authority and fear', 'liking', 'reciprocity'],
    xp: 20,
    explanation: `Pretexting is the backbone of all advanced social engineering. The pretext must be:
  - Believable: consistent with how your target organisation operates
  - Urgent: time pressure short-circuits rational evaluation (System 2 thinking)
  - Low-risk seeming: the ask must feel reasonable, not suspicious

IT Help Desk pretext script (red team awareness):
  "Hi, this is [NAME] from the IT Security Operations team. I'm calling because our monitoring
   system flagged your account for an unusual login from [CITY/COUNTRY] this morning.
   I need to verify your identity to determine if it was you or if we need to freeze the account.
   Can you confirm your username and the last password you remember setting?"

Why it works:
  Authority:  Caller claims IT authority over systems the target depends on
  Fear:       Account compromise is a credible threat
  Urgency:    "This morning" — the problem is happening NOW
  Reciprocity: "I'm trying to protect you" — the caller seems helpful

Variations of this pretext:
  Password expiration:  "Your password expires in 2 hours, I can extend it if you verify..."
  VPN access:          "Your VPN certificate needs reissuing, I need your current password to..."
  Payroll system:      "We're migrating payroll, I need to verify your bank details are correct..."
  Microsoft/Google:    Impersonate cloud provider support — huge trust, broad credibility

Defence:
  Call-back verification: never give info to inbound callers, hang up and call the official IT number
  Zero-trust identity: IT will NEVER ask for your password — train this relentlessly
  Callback culture: employees should always offer "let me call you back on the official number"`,
    flag: 'FLAG{pretext_anatomy_complete}'
  },

  {
    id: 'se-14',
    title: 'Phase 6 — Vishing: Voice Deepfake and AI-Enhanced SE',
    objective: `AI voice cloning has collapsed the cost and skill barrier for voice-based SE. Tools like ElevenLabs, Resemble AI, and open-source models can clone a voice from 30 seconds of audio — enough from a YouTube video, earnings call, or podcast. An attacker clones the CEO's voice and calls the CFO requesting an urgent wire transfer. This is a variant of Business Email Compromise (BEC) executed over voice. What term describes this AI-generated voice fraud targeting executives?`,
    hint: 'This is a type of deepfake fraud specifically using cloned audio, targeting high-value individuals.',
    answers: ['vishing', 'deepfake audio', 'voice deepfake', 'audio deepfake', 'synthetic voice fraud', 'ai voice fraud', 'clone fraud'],
    xp: 20,
    explanation: `AI-enhanced vishing is the fastest-growing social engineering threat vector in 2024-2026.

Real incidents:
  - A UK energy company CEO was defrauded of 220,000 EUR by an attacker using an AI voice clone
    of the parent company's CEO, requesting an urgent wire transfer
  - A Hong Kong finance firm lost $25M USD when an employee joined a video call with deepfaked
    executives who instructed him to transfer funds

How voice cloning works:
  1. Collect voice samples: LinkedIn videos, earnings calls, podcasts, YouTube interviews
  2. Feed to a voice cloning model: ElevenLabs (API), Coqui TTS, RVC (Retrieval Voice Conversion)
  3. Generate audio: type the script, output sounds like target
  4. Call via spoofed caller ID (spoofcard.com, SpoofBox, asterisk PBX)

Red team voice deepfake tools (for authorised awareness testing):
  ElevenLabs     — commercial, best quality, API available
  RVC            — open source retrieval-based voice conversion
  Tortoise TTS   — open source, high quality, slower
  Bark           — open source, supports emotional inflection

Detection methods:
  Listen for: unnatural pacing, robotic cadence, audio artefacts on consonants
  Liveness test: ask unexpected personal questions only the real person would know
  Call-back policy: any financial request over voice MUST be verified via a second channel
  Pause-and-verify: instil a culture where "I need to call you back to verify" is normal and expected

Organisational defence:
  Verbal codewords: executives and finance team agree on a pre-shared code word for urgent requests
  Dual approval: no wire transfer approved from single voice instruction regardless of who calls
  AI detection tools: Pindrop, Nuance, Resemble Detect for real-time call analysis`
  },

  {
    id: 'se-15',
    title: 'Phase 6 — Physical Security: Badge Cloning and RFID Attacks',
    objective: `Physical social engineering bridges the digital and physical worlds. Most corporate buildings use RFID/NFC proximity cards for access control. HID Prox cards (125kHz) — one of the most common formats — have no encryption and can be cloned silently from up to 90cm away using a Proxmark3. You walk past an employee, capture their card data, write it to a blank card, and now have a physical access clone. What open-source hardware tool is the standard for RFID card reading, writing, and cloning?`,
    hint: 'This tool was created by Jonathan Westhues and is named after a character from the Matrix.',
    answers: ['proxmark3', 'proxmark', 'proxmark3 rdv4', 'flipper zero'],
    xp: 25,
    explanation: `Physical access bypasses all digital controls — if you can get inside, game over.

Proxmark3 — RFID/NFC attack toolkit:
  pm3 → hw version                     Verify Proxmark is connected
  pm3 → lf search                      Scan for low-frequency (125kHz HID, EM4100) cards
  pm3 → lf hid read                    Read HID Prox card data
  pm3 → lf hid clone -r RAW_HEX        Clone to T5577 blank card
  pm3 → hf search                      Scan for high-frequency (13.56MHz Mifare, iCLASS)
  pm3 → hf mf autopwn                  Auto-attack Mifare Classic (broken crypto1)
  pm3 → hf iclass dump                 Dump iCLASS card sectors

Flipper Zero — portable multi-protocol tool:
  Reads and emulates HID, EM4100, Mifare, NFC
  Sub-GHz radio for garage doors, gate remotes
  Infrared transceiver for TVs and AV equipment
  Fits in a pocket — undetectable in social contexts

Proximity cloning technique (tailgating prep):
  1. Covert reader (e.g., Proxmark3 Easy in a backpack) captures card at ~90cm
  2. Copy HID facility code and card number to T5577 blank
  3. Present cloned card at target entrance — badge reader accepts it
  4. Once inside: plant a USB drop, access server rooms, photograph sensitive materials

Physical SE techniques:
  Tailgating:      Follow authorised person through a door (hold the door social norm)
  Piggybacking:    Same but with the target's awareness and manufactured consent
  Dumpster diving: Shredded documents, printer paper with passwords, hardware tags
  USB drop:        Label a USB "Q3 Salaries Confidential" and leave in parking lot
  Impersonation:   Hi-vis vest + clipboard = invisible in most facilities

Countermeasures: Mantraps (two-door airlocks), turnstiles, tailgating detection cameras, security escort culture, shred all documents, use iCLASS SE or SEOS (encrypted RFID).`,
    flag: 'FLAG{physical_se_complete}'
  },

  {
    id: 'se-16',
    title: 'Phase 6 — Measuring SE Effectiveness: Phishing Metrics',
    objective: `Running phishing simulations without measuring outcomes is pointless. The three metrics that matter most in a phishing campaign are: open rate (did they open the email?), click rate (did they click the link?), and credential submission rate (did they enter their password?). A well-designed awareness programme tracks these over time to show improvement. If 400 employees were sent a phishing simulation and 68 clicked the link, what is the click rate as a percentage?`,
    hint: 'Click rate = (clicks / total sent) multiplied by 100. 68 divided by 400, times 100.',
    answers: ['17', '17%', '17 percent'],
    xp: 15,
    explanation: `Phishing simulation metrics are how you justify the SE testing programme to leadership and measure training effectiveness.

Key phishing metrics:
  Open rate:               How many opened the email (shows subject line effectiveness)
  Click rate:              How many clicked a link (shows body content effectiveness)
  Credential submission:   How many entered data (shows landing page and trust effectiveness)
  Report rate:             How many reported it to security (your GOAL — should increase over time)
  Repeat offenders:        Who clicked multiple campaigns (needs targeted training)

Industry benchmarks (Proofpoint State of the Phish 2024):
  Average click rate without training:   ~33%
  Average click rate with good training: ~8-12%
  Goal for mature programmes:            Under 5%

GoPhish campaign results analysis:
  Results → Campaign Name → Timeline view
  Export CSV for import into Excel/Splunk/SIEM
  Group by department to find highest-risk teams

Reporting to leadership:
  Show trend over time: Month 1 = 28%, Month 3 = 19%, Month 6 = 11%
  Cost per click: If average breach cost is $4M and click rate is 11%, annual risk = $440K
  Training ROI: Training cost $50K → click rate down to 5% → risk reduction $240K

Phishing-resistant MFA:
  Even if credentials are submitted, FIDO2/WebAuthn hardware keys cannot be phished
  The token is bound to the legitimate domain — a phishing site gets no usable token
  Organisations should push toward hardware keys (YubiKey, Google Titan) for high-risk roles`
  },
]
