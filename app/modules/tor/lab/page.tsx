...
const steps: LabStep[] = [
  {
    ...
    xp: 20,
    explanation: 'systemctl status tor shows whether the Tor daemon is active. A running Tor service means your traffic can now be routed through the Tor network. On Debian/Ubuntu: sudo apt install tor && sudo systemctl start tor'
  },
  {
    id: 'tor-02',
    title: 'Locate the Tor Config File',
    objective: 'The main Tor config file controls all settings — ports, logging, hidden services. What is the full path to the torrc configuration file?',
    hint: 'Tor config lives in /etc/tor/ on most Linux systems.',
    answers: ['/etc/tor/torrc', 'etc/tor/torrc'],
    xp: 20,
    explanation: '/etc/tor/torrc is the primary configuration file. You edit this to set SocksPort, create hidden services, set up bridge relays, configure exit policies, and control logging behaviour.'
  },
  {
    id: 'tor-03',
    title: 'Verify SOCKS5 Proxy',
    objective: 'Tor exposes a SOCKS5 proxy on localhost:9050. Type the curl command to verify your external IP is routed through Tor using the SOCKS5 proxy.',
    hint: 'curl has a --socks5 flag. Use https://check.torproject.org/api/ip to verify.',
    answers: [
    ...
    xp: 30,
    explanation: '--socks5 localhost:9050 routes the curl request through Tor. The response {"IsTor":true} confirms your traffic exits via a Tor relay. This is the standard verification step before routing any sensitive traffic. For CLI tools that don\'t support SOCKS natively, use torsocks: torsocks wget https://target.onion'
  },
  {
    id: 'tor-04',
    title: 'Configure Hidden Service',
    objective: 'To create a .onion hidden service, add two directives to /etc/tor/torrc. What directive sets the hidden service DIRECTORY path? (the one ending in "Dir")',
    hint: 'The directive starts with "HiddenService" and sets the directory where Tor stores the private key and hostname.',
    answers: ['hiddenservicedir', 'HiddenServiceDir', 'HiddenServiceDir /var/lib/tor/hidden_service/'],
    xp: 25,
    explanation: 'The minimal hidden service config requires two lines in torrc:\n  HiddenServiceDir /var/lib/tor/hidden_service/\n  HiddenServicePort 80 127.0.0.1:8080\nOn first start, Tor generates an Ed25519 keypair and writes the v3 .onion address to hostname file. The port directive maps onion:80 to your local server on 8080.'
  },
  {
    id: 'tor-05',
    title: 'Read Your .onion Address',
    objective: 'After starting a hidden service, Tor writes the .onion hostname to a file inside HiddenServiceDir. What command reads the hostname file? (assume HiddenServiceDir is /var/lib/tor/hidden_service/)',
    hint: 'Use cat to read the file named "hostname" inside the hidden service directory.',
    answers: [
      'cat /var/lib/tor/hidden_service/hostname',
    ],
    flag: 'FLAG{onion_service_active}',
    xp: 35,
    explanation: 'The hostname file contains your v3 .onion address — a 56-character base32 string derived from the Ed25519 public key. Example: 3g2upl4pq6kufc4m.onion\nThis is your permanent address. Share it via PGP-signed messages to give others access. v3 onion addresses are quantum-resistant due to Ed25519 key length. To vanity-mine a custom prefix, use mkp224o: mkp224o ghost /var/lib/tor/hidden_service/'
  }
]
...
export default function TorLab() {
  ...
  // Restore guided completion from localStorage
  useEffect(() => {
  ...
  return (
    <div>
      ...
      {/* Progress banner */}
      <div style={{ background: 'rgba(0,255,65,0.04)', border: '1px solid rgba(0,255,65,0.15)', borderRadius: '6px', padding: '10px 16px', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
      ...
      <div style={{ marginBottom: '3rem' }}>
        ...
        <p style={{ color: '#6a8a6a', fontSize: '0.85rem', marginBottom: '1rem', lineHeight: 1.7, fontFamily: 'JetBrains Mono, monospace' }}>
          Step-by-step guided exercises covering Tor service management, SOCKS proxy configuration, and hidden service deployment.
        ...
        <div style={{ background: 'rgba(0,255,65,0.03)', border: '1px solid rgba(0,255,65,0.12)', borderRadius: '6px', padding: '1rem 1.25rem', marginBottom: '1.25rem', fontFamily: 'JetBrains Mono, monospace' }}>
          <div style={{ fontSize: '7px', color: '#1a4a1a', letterSpacing: '0.25em', marginBottom: '8px' }}>KEY CONCEPTS COVERED IN THIS LAB</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
            {['Tor service management', 'torrc configuration', 'SOCKS5 proxying', 'Hidden service setup', '.onion address generation', 'v3 onion vs v2', 'torsocks wrapper', 'mkp224o vanity addresses'].map(c => (
        ...
        {!freeLaunched ? (
          <div style={{ background: guidedDone ? 'rgba(0,255,65,0.04)' : '#020602', border: '1px solid ' + (guidedDone ? 'rgba(0,255,65,0.25)' : '#0a1a0a'), borderRadius: '10px', padding: '2.5rem', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.78rem', color: '#4a7a4a', marginBottom: '0.75rem', lineHeight: 1.7 }}>
              Free-form terminal sandbox for Tor & Dark Web
            </div>
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.72rem', color: '#3a6a3a', marginBottom: '2rem', lineHeight: 1.7 }}>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '2rem' }}>
              {[
                'systemctl tor commands',
                'torrc configuration',
                'SOCKS5 proxy testing',
                'Hidden service creation',
                'torsocks usage',
                'Circuit monitoring via nyx',
              ].map(feat => (
            ...
            <button
              ...
              style={{
                ...
                padding: '14px 40px',
                border: '1px solid ' + (guidedDone ? 'rgba(0,255,65,0.6)' : '#1a3a1a'),
                borderRadius: '6px',
                background: guidedDone ? 'rgba(0,255,65,0.12)' : 'transparent',
                color: guidedDone ? accent : '#1a3a1a',
                cursor: guidedDone ? 'pointer' : 'not-allowed',
                boxShadow: guidedDone ? '0 0 24px rgba(0,255,65,0.18)' : 'none',
                transition: 'all 0.2s',
              }}
            >
            ...
            {!guidedDone && (
              <div style={{ marginTop: '1rem', fontFamily: 'JetBrains Mono, monospace', fontSize: '7px', color: '#1a3a1a' }}>
                Complete all 5 guided steps above to unlock the free lab environment
              </div>
            )}
          </div>
        ) : (
          <div style={{ border: '1px solid ' + accent + '30', borderRadius: '10px', overflow: 'hidden', background: '#020802' }}>
            <FreeLabTerminal
              ...
              accent={accent}
              onClose={() => setFreeLaunched(false)}
            />
            {/* Ghost Agent hint strip */}
            <div style={{ padding: '8px 16px', background: '#030a03', borderTop: '1px solid ' + accent + '15', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '7.5px', color: '#3a7a3a' }}>
                You are in free practice mode. Ask <span style={{ color: accent }}>GHOST AGENT</span> (bottom-right) for hints, next steps, or deeper explanations on any Tor technique.
              </span>
            </div>
          </div>
        )}
      ...
      <div style={{ marginBottom: '2rem' }}>
        ...
        >
          {showKeywords ? '▼' : '▶'} QUICK REFERENCE — TOR COMMANDS
        </button>
        {showKeywords && (
          <div style={{ background: '#030a03', border: '1px solid #0d1f0d', borderRadius: '6px', padding: '1.25rem', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '8px' }}>
              {[
                ['sudo apt install tor', 'Install Tor on Debian/Ubuntu'],
                ['sudo systemctl start tor', 'Start Tor service'],
                ['systemctl status tor', 'Check Tor daemon status'],
                ['curl --socks5 localhost:9050 <url>', 'Route curl through Tor'],
                ['torsocks <command>', 'Force any app through Tor SOCKS'],
                ['cat /etc/tor/torrc', 'View Tor configuration'],
                ['HiddenServiceDir /var/lib/tor/hs/', 'Set hidden service directory in torrc'],
                ['HiddenServicePort 80 127.0.0.1:8080', 'Map onion port to local service'],
                ['cat /var/lib/tor/hidden_service/hostname', 'Read your .onion address'],
                ['mkp224o ghost /var/lib/tor/hs/', 'Generate vanity .onion address'],
                ['tor --version', 'Check Tor version'],
                ['nyx', 'Terminal monitor for Tor relay/circuits'],
              ].map(([cmd, desc]) => (
                <div key={cmd} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', padding: '6px 8px', background: '#020602', borderRadius: '4px' }}>
                  ...
                  <span style={{ color: '#4a7a4a', fontSize: '0.7rem' }}>{desc}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Footer nav */}
      <div style={{ paddingTop: '2rem', borderTop: '1px solid #0d1f0d', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
